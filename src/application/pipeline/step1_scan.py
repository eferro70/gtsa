...
step1_scan.py
-------------------
Script para análise estática de projetos Node/TypeScript, extraindo endpoints de APIs a partir do código-fonte.

Funcionalidades:
- Varre recursivamente o diretório informado, analisando arquivos .ts e .tsx (exceto arquivos/diretórios ignorados).
- Utiliza um parser AST (por padrão, ast_parser_node) para extrair endpoints e sumarizar a árvore sintática.
- Gera relatórios detalhados em JSON e Markdown, incluindo:
        - Lista de endpoints por arquivo
        - Resumo da análise
        - Log de erros
        - AST resumida

Parâmetros de linha de comando:
    -i, --input   Caminho para a raiz do projeto Node/TypeScript a ser analisado (obrigatório)
    --parser      Nome do arquivo do parser (sem .py) a ser utilizado (padrão: ast_parser_node)

Exemplo de uso:
        python3 step1_scan.py -i /caminho/para/projeto

Saídas:
- Cria um diretório output/ast/scan_<timestamp> com todos os relatórios e arquivos gerados para a execução.
"""
...
def main():
    ...
    # Adiciona pasta de parsers ao sys.path (src/infrastructure/parsers)
    PARSER_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../infrastructure/parsers'))
